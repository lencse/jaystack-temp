...
(function ($, window, mediator) {
    ...
    var $window, $body, $targetElement, $topElement, $bottomElement, $wrapperElement, $parentElement, $headerElement
    ,   oldTargetElementPosition
    ,   oldWindowScroll = 0
    ,   oldTotalTopElementHeights
    ,   options
    ,   oldTopElementTopOffset
    ,   defaultOptions = { topOffset: 0, topOffsetTab: 0, bottomOffset: 0 }
    ,   initialized = false
    ...
    function computePositions() {
        ...
        var upperLimit = Math.max(topElementTopOffset + topElementHeight, windowScroll) + ( MF.breakpoint.getActive() == 'tablet' ? options.topOffsetTab : options.topOffset )         // actual position of the top elements' bottom, or the top visible area of the window (whichever is lower)
        ...
        var currBreak = MF.breakpoint.getActive();
        var offsetCheck = 75;
        if ( !$('html').hasClass('touch') && currBreak != 'tablet' ) {
            offsetCheck = ( offsetCheck - options.topOffset < 0 ? offsetCheck : offsetCheck - options.topOffset );
        }
        ...
        if ($window.scrollTop() <= offsetCheck) {
        ...
        if($window.scrollTop() <= offsetCheck) {
            $targetElement.removeClass('got_stuck');
            $targetElement.css({
                'position': 'static',
                'top': ''
            });
            if ( $parentElement.hasClass('is_stuck') ) {
                $parentElement.removeClass('is_stuck');
            }
        } else {
            if ( $('html').hasClass('touch') || currBreak == 'tablet' || ( currBreak != 'tablet' && $('.header-wrapper__nav > .topbar').hasClass('sticky') ) ) {
                if ( $parentElement.hasClass('slide-in') ) {
                    $parentElement.css('opacity', 1);
                    $parentElement.removeClass('slide-in');
                }
                $parentElement.addClass('is_stuck');
                if ( $targetElement.css('position') != 'fixed' ) {
                    $targetElement.addClass('got_stuck');
                }
                else {
                    $targetElement.removeClass('got_stuck');
                }
    ...
    function init( pOptions ) {
        ...
        $parentElement = $wrapperElement.parent();
        $headerElement = $targetElement.find('.mf-shopby__nav__header');
        ...
        Breakpoints.on({ name: 'tablet',        matched: function() { enableStickyBehaviour(); } });
        ...
        options.topOffset = options.topOffset;
        options.topOffsetTab = options.topOffsetTab;
